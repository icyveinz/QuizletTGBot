...
class CallbackCardsTrainerFlipCondition(BaseFilter):
    async def __call__(self, callback_query: CallbackQuery) -> bool:
        ...
        try:
            ...
            if callback_query.data.startswith("FLIP:") and user_state == StatesEnum.TRAINS_CARDS.value:
                return True
...
class CallbackCardsTrainerMarkStudiedCondition(BaseFilter):
    async def __call__(self, callback_query: CallbackQuery) -> bool:
        ...
        try:
            user_repo = UserRepository()
            user_state = user_repo.get_user(user_id).state
            if callback_query.data.startswith("MARK_STUDIED:") and user_state == StatesEnum.TRAINS_CARDS.value:
                return True
        except Exception as e:
...
class CallbackCardsTrainerNextCondition(BaseFilter):
    async def __call__(self, callback_query: CallbackQuery) -> bool:
        ...
        try:
            user_repo = UserRepository()
            user_state = user_repo.get_user(user_id).state
            if callback_query.data.startswith("NEXT:") and user_state == StatesEnum.TRAINS_CARDS.value:
                return True
        except Exception as e:
