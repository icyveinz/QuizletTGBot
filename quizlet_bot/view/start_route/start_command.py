...
async def handle_create_cards_button(message: types.Message):
    user_id = message.from_user.id
    user_states[user_id] = {"state": "awaiting_front", "front": None}  # Set state and initialize temp data
...
async def handle_card_input(message: types.Message):
    ...
    if user_data["state"] == "awaiting_front":
        # Save the front text
        user_states[user_id]["front"] = message.text.strip()
        user_states[user_id]["state"] = "awaiting_back"
        await message.reply("Front side saved! Now, please enter the back side of the card.")

    elif user_data["state"] == "awaiting_back":
        # Save the back text
        ...
        try:
            ...
            await message.reply(f"Card created! Front: {front}, Back: {back}")
        except SQLAlchemyError as e:
