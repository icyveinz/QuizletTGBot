...
class SeenCardsRepository:
    ...
    async def mark_card_as_seen(self, user_id: str, card_id: int) -> bool:
        try:
            stmt = insert(SeenCardsEntity).values(
                related_user_id=user_id, related_card_id=card_id
    ...
    async def get_list_of_related_and_seen_cards(self, user_id: str) -> list[int]:
        try:
            result = await self.db.execute(
                select(SeenCardsEntity.related_card_id)
                .filter_by(related_user_id=user_id)
            )
